...
const Admin: React.FC = () => {
  ...
  return (
    <div className="container fade-in" style={{ maxWidth: '1000px', padding: '2rem 1rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <LayoutDashboard color="var(--accent-color)" />
          <h1 style={{ fontSize: '1.5rem' }}>SSLG Dashboard</h1>
        </div>
        <button onClick={handleLogout} className="button" style={{ background: '#ef4444', padding: '8px 16px', fontSize: '0.9rem' }}>
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', marginBottom: '3rem' }}>
          <div className="card" style={{ padding: '1.5rem' }}>
            <div style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '0.5rem' }}>Total Responses</div>
            <div style={{ fontSize: '2rem', fontWeight: 800 }}>{stats.totalResponses}</div>
          </div>
          {stats.categoryDistribution.map((item: any) => (
            <div key={item.category} className="card" style={{ padding: '1.5rem' }}>
              <div style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '0.5rem', textTransform: 'capitalize' }}>{item.category}</div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 700 }}>{item.count}</div>
                <div style={{ fontSize: '0.8rem', color: 'var(--accent-color)' }}>★ {item.avg_rating.toFixed(1)}</div>
              </div>
            </div>
          ))}
        </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: 'var(--text-secondary)', fontSize: '0.8rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <Clock size={14} /> {new Date(res.created_at).toLocaleDateString()}
                </div>
                <div>★ {res.rating}</div>
              </div>
            </div>
            <p style={{ marginBottom: '1rem', color: 'var(--text-primary)' }}>{res.message || <em style={{ color: 'var(--text-secondary)' }}>No message provided.</em>}</p>
            <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
              {res.is_anonymous ? 'Anonymous' : `Student ID: ${res.student_id || 'Not provided'}`}
            </div>
          </div>
        ))}
        {responses.length === 0 && (
