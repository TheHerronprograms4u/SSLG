...
app.get('/api/admin/stats', authenticateToken, (req: any, res: Response) => {
  try {
    const totalResponses = (db.prepare('SELECT count(*) as count FROM feedback').get() as any).count;
...
app.get('/api/admin/responses', authenticateToken, (req: any, res: Response) => {
  try {
